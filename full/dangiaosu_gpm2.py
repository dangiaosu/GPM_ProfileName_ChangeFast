...
def print_header():
    print("\033[1;36mCode Python đổi tên, đổi Proxy, đổi Group ID và cập nhật Note Profile GPM nhanh chóng by Đan Giáo Sư\033[0m")
    print("\n\033[1;33mChuyên lùa gà, nhưng vẫn ghét fake news\033[0m")
    print("\n\033[1;33mThông tin liên hệ:\033[0m")
    print("\033[1;34mTelegram: https://t.me/dangiaosu/\033[0m")
    print("\033[1;35mFacebook: https://fb.com/prof.danta/\033[0m")
    print("\033[1;32mZalo: https://zalo.me/0828092390/\033[0m\n")
...
def process_update_profiles(file_path, profiles):
    try:
        ...
        required_columns = ['ProfileName', 'NewName', 'NewGroupID', 'NewProxy', 'NewNote']
        if not all(col in df.columns for col in required_columns):
            raise ValueError("File Excel cần có các cột: 'ProfileName', 'NewName', 'NewGroupID', 'NewProxy', 'NewNote'")
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Đang cập nhật profiles..."):
        ...
        new_note = row['NewNote'] if pd.notna(row['NewNote']) else None
        ...
        if profile_id:
            ...
            if profile_info:
                update_data = {
                    ...
                    "note": new_note or profile_info.get('note', ''),
